...
def gradient_updates_adagrad(cost, params, learning_rate, eps=0.95):
...
def gradient_updates_adadelta(cost, params, learning_rate, eps=0.95, rho=1e-6):
    ...
    for param in params:
        ...
        dparam = -T.sqrt((param_update_2 + eps) / (param_update_1 + eps)) * gparam
        
        updates.append((param, param+dparam))
        updates.append((param_update_1, rho*param_update_1+(1. - rho)*(gparam ** 2)))
...
class DBN(object):
    ...
    def build_finetune_functions(self, datasets, batch_size, batch_size_t, batch_size_v, learning_rate):
        ...
        n_valid_batches /= batch_size_v
        n_test_batches = test_set_x.get_value(borrow=True).shape[0]
        n_test_batches /= batch_size_t
        ...
        #updates = self.create_optimization_updates(lr=learning_rate, max_norm=0.,method='adagrad')    
        updates = optimizer(self.finetune_cost, self.params, learning_rate=learning_rate,method='adadelta') #sgd-memotum
        ...
        test_score_i = theano.function(
            ...
            givens={
                self.x: test_set_x[
                    index * batch_size: (index + 1) * batch_size_t
                ],
                self.y: test_set_y[
                    index * batch_size: (index + 1) * batch_size_t
                ]
        ...
        valid_score_i = theano.function(
            ...
            givens={
                self.x: valid_set_x[
                    index * batch_size: (index + 1) * batch_size_v
                ],
                self.y: valid_set_y[
                    index * batch_size: (index + 1) * batch_size_v
                ]
...
def test_DBN(finetune_lr=1.0, pretraining_epochs=0,
