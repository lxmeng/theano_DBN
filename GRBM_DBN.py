...
from mnist_test import LogisticRegression, load_data
from mlp import HiddenLayer
from rbm import RBM
from grbm import GBRBM
from utils import zero_mean_unit_variance
from utils import normalize
from dbn import optimizer
...
#            updates.append((param, param - gparam * self.state_learning_rate.get_value()))
        ...
        train_fn = theano.function(inputs=[index],
              ...
              updates=updates,
              givens={self.x: train_set_x[index * batch_size[0]:
                                          (index + 1) * batch_size[0]],
                      self.y: train_set_y[index * batch_size[0]:
                                          (index + 1) * batch_size[0]]})

        test_score_i = theano.function([index], self.errors,
                 givens={self.x: test_set_x[index * batch_size[2]:
                                            (index + 1) * batch_size[2]],
                         self.y: test_set_y[index * batch_size[2]:
                                            (index + 1) * batch_size[2]]})

        valid_score_i = theano.function([index], self.errors,
              givens={self.x: valid_set_x[index * batch_size[1]:
                                          (index + 1) * batch_size[1]],
                      self.y: valid_set_y[index * batch_size[1]:
                                          (index + 1) * batch_size[1]]})
    ...
    def predict(self, test_set, batchsize):
...
def test_GRBM_DBN(finetune_lr=0.2, pretraining_epochs=2,
    ...
    dbn = GRBM_DBN(numpy_rng=numpy_rng, n_ins=28 * 28,
                hidden_layers_sizes=[1024, 1000, 1000],
    ...
    train_fn, validate_model, test_model = dbn.build_finetune_functions(
                datasets=datasets, batch_size=[batch_size,batch_size,batch_size],
...
def load_data_grbm(dataset):
    ...
    train_set_feature = train_set[0][0:,:]
    
    stat  = preprocessing.StandardScaler().fit(train_set_feature)
    train_set_feature1 = normalize(train_set_feature, stat.mean_, stat.std_)
    # Wudi added normalized data for GRBM
    #[train_set_feature1, Mean1, Std1] = zero_mean_unit_variance(train_set_feature)
    train_set_new_target = train_set[1][0:]
    train_set_x, train_set_y = shared_dataset( (train_set_feature1,train_set_new_target))

    valid_set_feature = valid_set[0]
    valid_set_feature = normalize(valid_set_feature, stat.mean_, stat.std_)
    valid_set_x, valid_set_y = shared_dataset((valid_set_feature,valid_set[1]))
    # test feature set
    test_set_feature = test_set[0]
    test_set_feature = normalize(test_set_feature, stat.mean_, stat.std_)
